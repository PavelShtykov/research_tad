...
class Layer0AttentionEager:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #     logger.info('\n\nEagerAttention forwfard was triggered!!!\n\n')
        #     self.is_logged = True
        ...
        # curr_qk: (B, H, I, D), (B, H, I, D) -> (B, H, I)
        curr_qk = (query * key_states).sum(-1, True) * scaling 

        # curr_qk_first: (B, H, I, D), (B, H, J, D) -> (B, H, I, J)
        curr_qk_first = torch.matmul(query, self.first_key_states.transpose(-2, -1)) * scaling 
        curr_qk_first = curr_qk_first + causal_mask


        concat_qk = torch.cat((curr_qk_first, curr_qk), dim=-1) 
        concat_qk = nn.functional.softmax(concat_qk, dim=-1, dtype=torch.float32).to(query.dtype)
        concat_qk = nn.functional.dropout(concat_qk, p=dropout, training=module.training)
        concat_qk = concat_qk * ((causal_mask == 0).any(-1) | (causal_mask == 0).any(-2))[..., None]

        curr_qk_first, curr_qk = concat_qk[..., :-1], concat_qk[..., -1:]

        # curr_qk_first: (B, H, I, J), (B, H, J, D) -> (B, H, I, D)
        curr_qk_first = torch.matmul(curr_qk_first, self.first_value_states)

        curr_qk = curr_qk * value_states # torch.einsum('bhi,bhid->bhid', curr_qk, value_states)
        attn_output = curr_qk_first + curr_qk
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        **kwargs,
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            causal_mask += min_dtype * torch.eye(*causal_mask.shape[-2:], dtype=dtype, device=device)
            ...
            if attention_mask is not None:
                ...
                padding_mask = (
                    ...
                    + attention_mask[:, None, None, :].to(causal_mask.device) * attention_mask[:, None, :, None].to(causal_mask.device)
                )
